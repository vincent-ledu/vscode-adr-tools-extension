...
function createNewAdr(
  ...
  srcAdrSanitizeName =
    lastIndex.padStart(4, "0") + "-" + sanitizedAdrName(srcAdrName) + ".md";
  ...
  let templateContent = fs.readFileSync(
    adrTemplatePath + path.sep + "index-recordname.md",
    "utf8"
  );
...
function addLink(srcAdrName, tgtFilePath, linkType) {
  console.log(
    "add link " + linkType + " " + srcAdrName + " to " + tgtFilePath
  );
...
function init(basedir, adrPath, adrTemplatePath, gitRepo) {
  ...
  if (!fs.existsSync(adrTemplatePath)) {
    ...
    cp.execSync("git clone " + gitRepo + " " + adrTemplatePath, function(
      err,
      stdout,
      stderr
    ) {
  ...
  let output = mustach.render(
    fs.readFileSync(
      adrTemplatePath + path.sep + "0000-record_architecture_decisions.md",
      "utf8"
    ),
    data
  );
  fs.writeFile(
    adrPath + path.sep + "0000-record_architecture_decisions.md",
    output,
    err => {
      if (err) {
